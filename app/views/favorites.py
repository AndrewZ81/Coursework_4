...
class FavoritesView(Resource):
    ...
    def delete(self, movie_id):
        ...
        :return: Удаляет запись из таблицы favorites
        """
                ...
                if i.id == movie_id:
                    try:
                        ...
                        delete_favorite = favorite.delete(). \
                            where(favorite.c.user_id == user.id). \
                            where(favorite.c.movie_id == movie_id)
                        conn.execute(delete_favorite)
                    except Exception:
                        return "Не удалось удалить из избранного", 404
                    else:
                        return "Удалено из избранного", 404
            return "Фильма с таким ID не существует", 404
        else:
            return "Такого пользователя не существует", 404
